...
import { UserCoupon } from "@/hooks/useCoupon";
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-3 mt-4">
          ...
          {coupons?.map((coupon) => {
            ...
            // 최소주문금액 충족 여부
            const canUse = subtotal >= (couponData.minOrderAmount ?? 0);

            // 만료는 보여주되 사용 불가
            const isAvailable = coupon.status === "AVAILABLE";

            // USED는 아예 숨김
            if (coupon.status === "USED") return null;

            // 실제 클릭 가능 여부
            const isClickable = canUse && isAvailable;
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  <p className="font-semibold">{couponData.name}</p>
                  <Gift className="w-5 h-5 text-orange-500" />
                </div>

                <p className="text-sm text-gray-600">
                  {(couponData.minOrderAmount ?? 0) > 0
                    ? t("order.couponDialog.minOrder", {
                        amount: (couponData.minOrderAmount ?? 0).toLocaleString(),
                      })
                ...
                {!canUse && (
                  <p className="text-xs text-red-500 mt-1">
                    {t("order.couponDialog.notEnough", {
                      remain: ((couponData.minOrderAmount ?? 0) - subtotal).toLocaleString(),
                    })}
                  </p>
                )}

                {coupon.status === "EXPIRED" && (
                  <p className="text-xs text-gray-500 mt-1"> {t("order.couponDialog.expired")}</p>
                )}
              </div>
            );
          })}
