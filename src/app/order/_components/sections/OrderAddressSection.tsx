...
export function OrderAddressSection({ onOpenAddressDialog }: OrderAddressSectionProps) {
  ...
  useEffect(() => {
    ...
    const defaultId = listData.find((a) => a.isDefault)?.id ?? listData[0]?.id ?? null;
    setValue("selectedAddressId", defaultId);
  }, [listData, selectedAddressId, setValue]);
